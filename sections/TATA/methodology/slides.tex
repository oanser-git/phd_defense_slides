...
\begin{frame}{Phase 1: Test-set Assessment}
	...
	\begin{minipage}[t]{0.48\textwidth}
		\begin{itemize}
			...
			\item Proximity, diversity, and scarcity to characterize where \(\mathcal{Z}_{\text{test}}\) lies in latent space relative to \(\mathcal{Z}_{\text{train}}\) clusters.
...
% --- Frame kept for possible later reuse ---
...
% 		      \begin{itemize}
% 			      \item<4-> Models protocol interactions, offering more realism.
...
% 			      \item<6-> Emits legitimate traffic across multiple protocols (HTTP/HTTPS, SSH, SMTP, DNS,
...
\begin{frame}[t]{Phase 2: Targeted Augmentation}
	...
	\begin{itemize}
		\item Start from Preliminary \& Phase 1 outputs
		\item Configurable live testbed for realistic traffic generation.
		\item Legitimate multi-protocol traffic (HTTP/HTTPS, SSH, SMTP, DNS, \dots)
		      and controlled attack traffic.
		\item []
		\item \textbf{Training workflow}:
		      \[
			      \begin{aligned}
		      ...
		      %      \end{aligned}
		      %    \]
		\item Apply \(\pi^\ast\) to augment any \(\mathcal{D}_{\text{test}}\) and target
		      under-covered \(\mathcal{Z}\) regions (low \(D,P,S\)).
	\end{itemize}
\end{frame}
