...
\begin{frame}{Roadmap of the presentation}
		...
		\begin{beamercolorbox}[wd=\linewidth,sep=0pt,rounded=true]{roadmap card dim}
			...
			\begin{minipage}[t][4.2cm][t]{\linewidth}
				...
				\hspace*{0.03\linewidth}\parbox{0.94\linewidth}{%
					...
					\textbullet\ Context change handling\\[0.45em]
					\textbullet\ Fast NIDS reconfiguration\\[0.45em]
					\textbullet\ Meta-learning-based auto-configuration}
			\end{minipage}
		\end{beamercolorbox}
	...
	{\fontsize{8}{5.2}\selectfont
		\parbox{0.99\linewidth}{\centering
			\textcolor{black!35}{\emph{Auto-tuning of Hyper-parameters for Detecting Network Intrusions via Meta-learning} (NOMS 2023, AnNet);\\
...
\begin{frame}{Focusing on \(\mathcal{D}_{\text{test}}\) and its relation to \(\mathcal{D}_{\text{train}}\)}
	...
	\begin{columns}[T,onlytextwidth]
		...
		\begin{itemize}
			\item Robust evaluation needs hard and corner cases.
			\item Pre-deployment evaluation relies on \(\mathcal{D}_{\text{test}}\).
			\item Question: does \(\mathcal{D}_{\text{test}}\) really stress the NIDS?
			\item \(\mathcal{D}_{\text{test}}\) must be assessed relative to \(\mathcal{D}_{\text{train}}\).
		\end{itemize}
...
\newcommand{\gOneFaded}[1]{\colorbox{blue!7}{\textcolor{black!40}{#1}}}
\newcommand{\gTwoFaded}[1]{\colorbox{green!9}{\textcolor{black!40}{#1}}}
\newcommand{\gThreeFaded}[1]{\colorbox{orange!18}{\textcolor{black!40}{#1}}}
...
\newcommand{\muted}[1]{\textcolor{black!35}{#1}}
\newcommand{\fadedcmark}{\textcolor{black!35}{\faCheck}}
\newcommand{\fadedxmark}{\textcolor{black!35}{\faTimes}}
\newcommand{\ncovlim}{%
	\parbox[t]{\linewidth}{%
...
\newcommand{\ncovlimmuted}{%
	\parbox[t]{\linewidth}{%
		\muted{Model access (\limMA) + NN-only (\iconNN) +}\\
		\muted{Ignores train--test alignment (\limTA)}}}
\newcommand{\ncovlimGOne}{%
	\parbox[t]{\linewidth}{%
		Model access (\gOne{\limMA}) + NN-only (\gOne{\iconNN}) +\\
		\muted{Ignores train--test alignment (\limTA)}}}
\newcommand{\ncovlimGTwo}{%
	\parbox[t]{\linewidth}{%
		\muted{Model access (\limMA) + NN-only (\iconNN) +}\\
		Ignores train--test alignment (\gTwo{\limTA})}}
\newcommand{\surplimmuted}{\muted{\limMA \; + \; \iconNN \; + \; Proximity-only focus (\limPR)}}
\newcommand{\surplimGOne}{\gOne{\limMA} \; + \; \gOne{\iconNN} \; + \; \muted{Proximity-only focus (\limPR)}}
\newcommand{\surplimGTwo}{\muted{\limMA \; + \; \iconNN \; + \;} Proximity-only focus (\gTwo{\limPR})}
\newcommand{\mutlimtopmuted}{\muted{\limMA \; + \; \limTA \; + \; Model-change sensitive (\limMC)}}
\newcommand{\mutlimtopGOne}{\gOne{\limMA} \; + \; \muted{\limTA} \; + \; Model-change sensitive (\gOne{\limMC})}
\newcommand{\mutlimtopGTwo}{\muted{\limMA} \; + \; \gTwo{\limTA} \; + \; \muted{Model-change sensitive (\limMC)}}
\newcommand{\mutlimbotmuted}{\muted{Retrain per dataset (\limRT) \;+\; Synthetic data (\limSY)}}
\newcommand{\mutlimbotGThree}{Retrain per dataset (\gThree{\limRT}) \;+\; Synthetic data (\gThree{\limSY})}
\newcommand{\manuallimmuted}{\muted{Unguided augmentation (\limMU)}}
\newcommand{\manuallimGThree}{Unguided augmentation (\gThree{\limMU})}
...
\begin{frame}[c]{Related Work}
	...
	% -------- Final table / gap focus sequence --------
	\only<4>{%
		\begin{tabular}{@{}>{\raggedright\arraybackslash}p{3.9cm} c c p{7.6cm}@{}}
			\toprule
			\textbf{Method category}          & \textbf{RQ1}                            & \textbf{RQ2} &
			...
			\\
			\midrule
			\muted{Neuron coverage}          & \cmark                                  & \xmark       & \ncovlimmuted \\[0.45em]
			\muted{Surprise coverage}        & \cmark                                  & \xmark       & \surplimmuted \\[0.45em]
			\multirow{2}{=}{\muted{Mutation testing}} & \multirow{2}{*}{\cmark} &
			\multirow{2}{*}{\cmark}
				                                  &
			\mutlimtopmuted
			\\
				                                  &                                         &              & \mutlimbotmuted \\[0.45em]
			Manual (Flood et al., 2024)       & \xmark                                  & \cmark
				                                  & Unguided augmentation (\limMU)
			\\
			\bottomrule
		\end{tabular}
		...
		\begin{beamercolorbox}[rounded=true,shadow=true,wd=\linewidth,sep=0.8ex]{takebox}
			\gOneFaded{\textbf{G1}}\;\muted{Model-related gaps}\quad
			\gTwoFaded{\textbf{G2}}\;\muted{Shortcomings in assessing \(\mathcal{D}_{\text{test}}\) $\rightarrow$ RQ1}\quad
			\gThreeFaded{\textbf{G3}}\;\muted{Weaknesses in \(\mathcal{D}_{\text{test}}\) augmentation workflows $\rightarrow$ RQ2}
		\end{beamercolorbox}
	}
	...
	\only<5>{%
		\begin{tabular}{@{}>{\raggedright\arraybackslash}p{3.9cm} c c p{7.6cm}@{}}
			...
			Surprise coverage                 & \cmark                                  & \xmark       & \surplimGOne \\[0.45em]
			\multirow{2}{=}{Mutation testing} & \multirow{2}{*}{\cmark}                 & \multirow{2}{*}{\cmark} & \mutlimtopGOne
			\\
				                                  &                                         &              & \mutlimbotmuted \\[0.45em]
			\muted{Manual (Flood et al., 2024)} & \fadedxmark                           & \fadedcmark  & \manuallimmuted
			\\
			\bottomrule
		\end{tabular}
		...
		\begin{beamercolorbox}[rounded=true,shadow=true,wd=\linewidth,sep=0.8ex]{takebox}
			\gOne{\textbf{G1}}\;Model-related gaps\quad
			\muted{\textbf{G2}\;Shortcomings in assessing \(\mathcal{D}_{\text{test}}\) $\rightarrow$ RQ1}\quad
			\muted{\textbf{G3}\;Weaknesses in \(\mathcal{D}_{\text{test}}\) augmentation workflows $\rightarrow$ RQ2}
		\end{beamercolorbox}
	}
	...
	\only<6>{%
		\begin{tabular}{@{}>{\raggedright\arraybackslash}p{3.9cm} c c p{7.6cm}@{}}
			...
			\midrule
			Neuron coverage                   & \cmark                                  & \xmark       & \ncovlimGTwo \\[0.45em]
			Surprise coverage                 & \cmark                                  & \xmark       & \surplimGTwo \\[0.45em]
			\multirow{2}{=}{Mutation testing} & \multirow{2}{*}{\cmark}                 & \multirow{2}{*}{\cmark} & \mutlimtopGTwo
			\\
				                                  &                                         &              & \mutlimbotmuted \\[0.45em]
			\muted{Manual (Flood et al., 2024)} & \fadedxmark                           & \fadedcmark  & \manuallimmuted
			\\
		...
		\vspace{0.8em}
		\begin{beamercolorbox}[rounded=true,shadow=true,wd=\linewidth,sep=0.8ex]{takebox}
			\muted{\textbf{G1}\;Model-related gaps}\quad
			\gTwo{\textbf{G2}}\;Shortcomings in assessing \(\mathcal{D}_{\text{test}}\)
			$\rightarrow$ RQ1\quad
			\muted{\textbf{G3}\;Weaknesses in \(\mathcal{D}_{\text{test}}\) augmentation workflows $\rightarrow$ RQ2}
		\end{beamercolorbox}
	...
	\only<7->{%
		\begin{tabular}{@{}>{\raggedright\arraybackslash}p{3.9cm} c c p{7.6cm}@{}}
			...
			\\
			\midrule
			\muted{Neuron coverage}          & \fadedcmark                             & \fadedxmark  & \ncovlimmuted \\[0.45em]
			\muted{Surprise coverage}        & \fadedcmark                             & \fadedxmark  & \surplimmuted \\[0.45em]
			\multirow{2}{=}{Mutation testing} & \multirow{2}{*}{\cmark} & \multirow{2}{*}{\cmark} & \mutlimtopmuted
			\\
				                                  &                                         &              & \mutlimbotGThree \\[0.45em]
			Manual (Flood et al., 2024)       & \xmark                                  & \cmark
				                                  & \manuallimGThree
			\\
			\bottomrule
	...
	\only<1>{\begin{beamercolorbox}[rounded=true,shadow=true,wd=\linewidth,sep=1ex]{defbox}
			...
			\textbf{Fraction of neurons} whose activation exceeds a \textbf{preset threshold}
			at least once across the test set; \textbf{higher} $\rightarrow$
			explore more of the model’s \textbf{internal}
			logic.
		\end{beamercolorbox}}
	...
	\only<2>{\begin{beamercolorbox}[rounded=true,shadow=true,wd=\linewidth,sep=1ex]{defbox}
			...
			Measures how broadly the test set \textbf{differs from}
			what the model saw in training; \textbf{higher} $\rightarrow$ the test set includes
			\textbf{highly surprising inputs}.
		\end{beamercolorbox}}

	\only<3>{\begin{beamercolorbox}[rounded=true,shadow=true,wd=\linewidth,sep=1ex]{defbox}
			\scriptsize
			\textbf{Mutation testing:}
			\textbf{Mutates the model and/or its training setup}
			(e.g., weights, architecture, or hyperparameters), then checks if the test set can
			\textbf{distinguish the mutant from the original model}.
		\end{beamercolorbox}}
...
\input{sections/TATA/methodology/slides}
...
\input{sections/TATA/results/slides}
